...
class VmwareRemoteIso(ModuleRestBase):
    def __init__(self, module):
        ...
        if self.params["src"]:
            ...
            self.source_is_iso = os.path.isdir(self.params["src"])
    ...
    # TODO: fix stupidness with filemap
    def get_file_map(self):
        if not self.source_is_iso:
            item_filename = os.path.basename(self.params["src"])
            return {item_filename: self.params["src"]}

        file_map = {}
        iso_dir = os.path.abspath(self.params["src"])
        for file_name in os.listdir(iso_dir):
            if file_name.endswith(".iso"):
                file_map[file_name] = os.path.join(iso_dir, file_name)
        return file_map
    ...
    def __create_file_spec(self, name, path):
        ...
        if self.params["checksum_algorithm"] and not self.source_is_iso:
